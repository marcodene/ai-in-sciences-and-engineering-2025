...
class StaticTrainer(BaseTrainer):
    ...
    def init_dataset(self, dataset_config):
        ...
        strategy = getattr(self.model_config, 'tokenization_strategy', 'grid')
        geometry_aware = getattr(self.model_config, 'geometry_aware_sampling', True)
        # Use seed from setup config if available, otherwise model config, otherwise default
        seed = getattr(self.config.setup, 'seed', getattr(self.model_config, 'random_seed', 42))

        # Extract radius parameters from MAGNO config
        magno_config = self.model_config.args.magno
        radius_mode = getattr(magno_config, 'radius_mode', 'fixed')
        dynamic_k = getattr(magno_config, 'dynamic_radius_k', 10)
        dynamic_alpha = getattr(magno_config, 'dynamic_radius_alpha', 1.2)
        
        # Extract physical points for geometry-aware sampling
        physical_points = None
        if strategy == 'random' and geometry_aware:
            # Get physical coordinates from training split
            x_train = data_splits['train']['x']
            if is_variable_coords:
                # For variable coords, take first sample's coordinates
                physical_points = x_train[0]  # [N_physical, coord_dim]
            else:
                # For fixed coords, x_train is already the mesh
                physical_points = x_train  # [N_physical, coord_dim]
            print(f"Extracted {len(physical_points)} physical points for geometry-aware sampling")
        ...
        # Prepare GraphBuilder if needed (Dynamic FX or VX)
        self.fixed_graphs = None
        if radius_mode == 'dynamic' or is_variable_coords:
             # We need graph builder
        ...
        if is_variable_coords:
            self._init_variable_coords_mode(data_splits, radius_mode, dynamic_k, dynamic_alpha)
        else:
            self._init_fixed_coords_mode(data_splits, radius_mode, dynamic_k, dynamic_alpha)
        
        print("Dataset initialization complete.")
    
    def _init_variable_coords_mode(self, data_splits, radius_mode='fixed', dynamic_k=10, dynamic_alpha=1.2):
        """Initialize for variable coordinates mode."""
        print("Setting up variable coordinates mode...")
        
        # Graph builder already initialized in init_dataset
        ...
        all_graphs = self.graph_builder.build_all_graphs(
            ...
            # Pass extra args via kwargs if supported or extend method. 
            # Note: build_all_graphs in GraphBuilder might need update or we pass to build_graphs_for_split inside it
            # Let's assume we updated build_all_graphs too or we update it now in graph_builder.py
            # For now I will pass them as kwargs and ensure graph_builder handles them
    ...
    def _init_fixed_coords_mode(self, data_splits, radius_mode='fixed', dynamic_k=10, dynamic_alpha=1.2):
        ...
        if radius_mode == 'dynamic':
            print("Precomputing dynamic radius graphs for fixed coordinates...")
            # We use x_coord which is [Nodes, 2] in train split
            # Wrap it to be [1, Nodes, 2] for graph builder
            x_train = data_splits['train']['x']
            # if x_train is [Nodes, 2], make it [1, Nodes, 2]
            if x_train.dim() == 2:
                x_train_sample = x_train.unsqueeze(0)
            else:
                x_train_sample = x_train[0:1] # already batched maybe? Usually fx mode x is [Nodes, 2]
            
            gno_radius = getattr(self.model_config.args.magno, 'radius', 0.033)
            scales = getattr(self.model_config.args.magno, 'scales', [1.0])
            
            enc_graphs, dec_graphs = self.graph_builder.build_graphs_for_split(
                 x_data=x_train_sample,
                 latent_queries=self.latent_tokens_coord,
                 gno_radius=gno_radius,
                 scales=scales,
                 radius_mode=radius_mode,
            ...
            self.fixed_graphs = {
                 'encoder': enc_graphs[0], # List of scales
                 'decoder': dec_graphs[0]
            }
    ...
    def _train_step_fixed_coords(self, batch):
        ...
        # Get precomputed graphs from self.fixed_graphs if available
        encoder_nbrs = self.fixed_graphs['encoder'] if self.fixed_graphs else None
        decoder_nbrs = self.fixed_graphs['decoder'] if self.fixed_graphs else None
