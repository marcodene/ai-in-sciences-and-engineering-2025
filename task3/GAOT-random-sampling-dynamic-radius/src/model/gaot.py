    ...
    def __init__(self,
        ...
        if coord_dim == 2:
            if len(latent_tokens_size) == 2:
                # Grid strategy: explicit (H, W)
                self.H = latent_tokens_size[0]
                self.W = latent_tokens_size[1]
            elif len(latent_tokens_size) == 1:
                # Random strategy: single N, infer square-like dimensions for patching
                N = latent_tokens_size[0]
                # Use sqrt(N) as both H and W for patch-based processing
                side = int(N ** 0.5)
                self.H = side
                self.W = side
                print(f"Random tokenization: {N} tokens, using {side}×{side} for patch processing")
            else:
                raise ValueError(f"For 2D, latent_tokens_size must have 1 or 2 dimensions, got {len(latent_tokens_size)}")
            self.D = None
        else:  # 3D
            if len(latent_tokens_size) == 3:
                # Grid strategy: explicit (H, W, D)
                self.H = latent_tokens_size[0]
                self.W = latent_tokens_size[1] 
                self.D = latent_tokens_size[2]
            elif len(latent_tokens_size) == 1:
                # Random strategy: single N, infer cube-like dimensions
                N = latent_tokens_size[0]
                side = int(N ** (1/3))
                self.H = side
                self.W = side
                self.D = side
                print(f"Random tokenization: {N} tokens, using {side}×{side}×{side} for patch processing")
            else:
                raise ValueError(f"For 3D, latent_tokens_size must have 1 or 3 dimensions, got {len(latent_tokens_size)}")
