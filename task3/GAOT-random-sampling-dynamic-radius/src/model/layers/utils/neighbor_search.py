...
class NeighborSearch(nn.Module):
    ...
    def forward(self, data, queries, radius):
        ...
        """
        # If radius is a tensor (dynamic radius), we force fallback to native implementation
        # because other backends (like torch_cluster) typically expect scalar radius
        if isinstance(radius, torch.Tensor) and radius.numel() > 1:
             if self.method != 'native':
                 # Only warn once if not already using native
                 # warnings.warn("Dynamic radius (tensor) detected. Forcing native neighbor search implementation.")
                 pass
