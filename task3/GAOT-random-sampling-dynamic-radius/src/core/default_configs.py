...
class ModelConfig:
    ...
    latent_tokens_size: Tuple[int, int] = (64, 64)                 # Latent token dimensions (H,W) or (H,W,D) for grid, (N,) for random
    tokenization_strategy: str = "grid"                            # Tokenization strategy: ["grid", "random"]
    geometry_aware_sampling: bool = True                           # Use geometry-aware sampling for random strategy
    random_seed: Optional[int] = None                              # Random seed for token generation (None=use setup.seed)
