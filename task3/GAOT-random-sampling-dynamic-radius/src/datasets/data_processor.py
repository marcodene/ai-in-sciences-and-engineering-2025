...
class DataProcessor:
    ...
    def generate_latent_queries(self, token_size: Tuple[int, ...],
                               strategy: str = 'grid',
                               physical_points: Optional[torch.Tensor] = None,
                               geometry_aware: bool = True,
                               seed: int = 42) -> torch.Tensor:
        """
        Generate latent query points using different tokenization strategies.
        
        Args:
            token_size: For grid: (N_x, N_y) or (N_x, N_y, N_z)
                       For random: (N_tokens,) or just N_tokens as int
            strategy: 'grid' (structured meshgrid) or 'random' (random sampling)
            physical_points: Physical mesh points for geometry-aware random sampling
            geometry_aware: If True and physical_points provided, sample from physical points
            seed: Random seed for reproducibility in random sampling
            
        Returns:
            latent_queries: [N, coord_dim] tensor of token positions (scaled to [-1, 1])
        """
        print(f"Generating latent queries with strategy: {strategy}")
        
        if strategy == 'random':
            latent_queries = self._generate_random_tokens(
                token_size, physical_points, geometry_aware, seed
            )
        else:  # 'grid' (default)
            latent_queries = self._generate_grid_tokens(token_size)
        
        # Initialize and apply coordinate scaler
        ...
        print(f"Generated {len(latent_queries)} latent query tokens with shape {latent_queries.shape}")
        
        return latent_queries
    
    def _generate_grid_tokens(self, token_size: Tuple[int, ...]) -> torch.Tensor:
        """
        Generate tokens on a regular structured grid (Strategy I - Baseline).
        
        Args:
            token_size: (N_x, N_y) for 2D or (N_x, N_y, N_z) for 3D
            
        Returns:
            latent_queries: [N, coord_dim] grid positions
        """
        ...
        if len(token_size) == 2:
            ...
            raise ValueError(f"Unsupported token_size dimensions for grid: {len(token_size)}")
        
        return latent_queries
    
    def _generate_random_tokens(self, token_size: Union[int, Tuple[int, ...]],
                                physical_points: Optional[torch.Tensor],
                                geometry_aware: bool,
                                seed: int) -> torch.Tensor:
        """
        Generate tokens via random sampling (Strategy II).
        
        Args:
            token_size: Number of tokens (int or tuple with single element)
            physical_points: Physical mesh points [N_physical, coord_dim] for sampling
            geometry_aware: If True, sample from physical_points; else uniform in bbox
            seed: Random seed
            
        Returns:
            latent_queries: [N_tokens, coord_dim] random positions
        """
        # Extract number of tokens
        if isinstance(token_size, int):
            n_tokens = token_size
        elif isinstance(token_size, (list, tuple)):
            if len(token_size) == 1:
                n_tokens = token_size[0]
            else:
                raise ValueError(
                    f"Random sampling requires single value for token_size, got {token_size}. "
                    f"Use [N] or N for random sampling, (N_x, N_y) for grid."
                )
        else:
            raise TypeError(f"token_size must be int or tuple, got {type(token_size)}")
        
        # Geometry-aware sampling: sample from actual physical points
        if geometry_aware and physical_points is not None:
            if len(physical_points) < n_tokens:
                print(f"Warning: Requested {n_tokens} tokens but only {len(physical_points)} "
                      f"physical points available. Falling back to uniform sampling.")
                return self._uniform_random_sampling(n_tokens, seed)
            
            print(f"  Using geometry-aware sampling from {len(physical_points)} physical points")
            generator = torch.Generator().manual_seed(seed)
            indices = torch.randperm(len(physical_points), generator=generator)[:n_tokens]
            latent_queries = physical_points[indices].clone()
            
        else:
            # Uniform random sampling in bounding box
            latent_queries = self._uniform_random_sampling(n_tokens, seed)
        
        return latent_queries
    
    def _uniform_random_sampling(self, n_tokens: int, seed: int) -> torch.Tensor:
        """
        Uniform random sampling in physical domain bounding box.
        ...
        Returns:
            latent_queries: [n_tokens, coord_dim] positions
        """
        phy_domain = self.metadata.domain_x
        coord_dim = len(phy_domain[0])
        
        print(f"  Using uniform random sampling in bounding box")
        
        generator = torch.Generator().manual_seed(seed)
        latent_queries = torch.rand(n_tokens, coord_dim, generator=generator, dtype=self.dtype)
        
        # Scale from [0, 1] to [domain_min, domain_max]
        for dim in range(coord_dim):
            x_min = phy_domain[0][dim]
            x_max = phy_domain[1][dim]
            latent_queries[:, dim] = latent_queries[:, dim] * (x_max - x_min) + x_min
